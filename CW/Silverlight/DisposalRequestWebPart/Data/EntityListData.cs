...
namespace CAS.SmartFactory.CW.Dashboards.DisposalRequestWebPart.Data
{
  ...
  {
    /// <summary>
    /// Initializes a new instance of the EntityListData class.
    /// </summary>
    /// <param name="tEntity">The attribute entity.</param>
      ...
      foreach ( var item in m_EntitieAssociations )
      {
        ...
        switch ( _entity.EntityState )
        {
          case EntityState.ToBeInserted:
            if ( item.Value != null )
              throw new ArgumentException( "Value is not null: Inconsistent content of association table, ToBeInserted should not has associated ListItem" );
            ...
            _newListItem.Update();
            m_DataContext.ReloadNewListItem( _newListItem );
            _cntrNew++;
            break;
